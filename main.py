...
@bot.command(description="Gera uma imagem com Portinari AI")
async def portinari_paint(ctx, prompt: discord.Option(str)):
...
@bot.command(description="Upscale one of images generated by Portinari")
async def portinari_upscale(ctx, index: discord.Option(int), reset_target : discord.Option(bool) =True):
    ...
    if Globals.targetID == "":
        await ctx.respond(
            'You did not set target. To do so reply to targeted message with "$portinari-here"'
        )
        return
...
@bot.command(description="Upscale to max targetted image (should be already upscaled using portinari_upscale)")
async def portinari_upscale_to_max(ctx):
    if Globals.targetID == "":
        await ctx.respond(
            'You did not set target. To do so reply to targeted message with "$portinari-here"'
        )
...
async def portinari_variation(ctx, index: discord.Option(int), reset_target : discord.Option(bool) =True):
    if (index <= 0 or index > 4):
        await ctx.respond("Invalid argument, pick from 1 to 4")
        return
...
async def on_message(message):
    ...
    if "$portinari-here" in message.content and message.content[0] == '$':
        try:
